...
// JACOALEX NEW CARD FUNCTIONS 

// BUG: 4 cards instead of 3
...
// BUG: Draw a card before checking if deck is empty (should cause a crash)
...
// BUG: forget to check to see if pile is in play
int handle_card_embargo(struct gameState *state, int handPos, int currentPlayer, int choice1)
{
  //+2 Coins
...
// BUG: trash flag accidently set when discarding outpost card
...
// BUG: removes two actions instead of adds two
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  {
  case adventurer:
    return handle_card_adventurer(drawntreasure, state, currentPlayer, temphand, z, cardDrawn);
  ...
  case smithy:
    return handle_card_smithy(currentPlayer, state, handPos);

  case village:
    return handle_card_village(currentPlayer, state, handPos);
  ...
  case embargo:
    return handle_card_embargo(state, handPos, currentPlayer, choice1);

  case outpost:
    return handle_card_outpost(state, handPos, currentPlayer);
